module Game = struct
	...
	type level = {
		...
		production: resources; (* En unités par minute *)
		storage: resources
	}
	type structure = {
		levels: level array;
	...
	(* Ville *)
	type city = {
		width: int; (* Longueur de la ville *)
		cells: cell array;
		mutable storage: resources; (* Totaux des valeurs des bâtiments *)
		mutable production: resources;
		mutable availableResources: resources;
		mutable time: float;
		mutable state: state;
	...
	module Structures = struct
			...
			levels = [|
				{
					...
					production = [
						{name = Resources.water; amount = 0.5};
					];
					storage = [
						{name = Resources.water; amount = 100.};
					];
				};
				{
					...
					storage = [
						{name = Resources.water; amount = 500.};
					];
				};
			|];
			width = 1;
			...
			levels = [|
				{
					...
					production = [
						{name = Resources.oil; amount = 0.5};
					];
					storage = [
						{name = Resources.oil; amount = 150.};
					];
				}
			|];
			width = 1;
		...
		let lumberCamp = {
			levels = [|{
				...
				production = [
					{name = Resources.wood; amount = 0.5};
				];
				storage = [
				...
				production = [
					{name = Resources.wood; amount = 1.5};
				];
				storage = [
					{name = Resources.wood; amount = 200.};
				];
		...
		let rocketStation = {
			levels = [|{
				...
				production = [
					{name = Resources.electricity; amount = -1.};
				];
				storage = [
					{name = Resources.oil; amount = 500.};
				];
		...
		let windTurbine = {
			levels = [|{
				...
				storage = [
					{name = Resources.electricity; amount = 10.};
				];
			}|];
			width = 1;
			editable = true;
	...
	module Calculations = struct
		...
		let sum_resources listOfResources =
			...
			List.iter
			(
				fun ressources -> List.iter
				(fun r -> sum.(r.name -1) <- sum.(r.name -1) +. r.amount) ressources
			) listOfResources;
			Array.to_list
				(Array.mapi (fun i amount -> {name = i+1; amount}) sum)

		let resources_produced production time =
			List.map
				(fun res -> {
					name = res.name;
					amount = res.amount *. time /. 60.
				})
				production
	...
	module City = struct
		...
		let new_worldStructure (structure, level) x player's tBW =
			...
			if structure = Structures.cityHall then
				wS.initialResources <- [{name = Resources.villagers; amount = Calculations.get_resource s.initialResources Resources.villagers}]
			else
				wS.initialResources <- s.initialResources;
			wS.storage <- s.storage;
			if not wS.player's then
				wS.availableResources <- s.initialResources;
			wS
		...
		let find_worldStructure city structure x offsets =
			List.find_opt
				...
				(
					List.concat (
						List.map
							(fun i -> (get city.cells (x+i)).ground)
							offsets
					)
				)
		...
		let select city {x; overground} =
			if (get city.cells x).explored <> Yes then
				city.interactions.selection <- []
			else (
				city.interactions.selectionCoords <- Some {x=modpos x city.width; overground};
				city.interactions.selection <- List.concat [
					(List.find_all
		...
		let update_worldStructure city (wS: worldStructure) time =
			...
			|| i = Structures.derrick
			|| i = Structures.lumberCamp -> (
				let wSo = match i with
					| i when i = Structures.well -> find_worldStructure city Structures.groundwater wS.coords.x [0]
					| i when i = Structures.derrick -> find_worldStructure city Structures.oilDeposit wS.coords.x [0]
					| i when i = Structures.lumberCamp -> find_worldStructure city Structures.forest wS.coords.x [-1; 1; -2; 2; -3; 3]
					| _ -> failwith "ah." in
				match wSo with
				| Some deposit -> (
					let dt = min
						(production_time city prod deposit.availableResources time true)
						(production_time city prod city.availableResources time false) in
					wS.working <- not (dt = 0.);
					let produced = List.map
						(fun res -> {name = res.name; amount = res.amount *. dt})
						prod in
					deposit.availableResources <- List.map (
						fun res -> {
							name = res.name;
							amount = Calculations.get_resource deposit.availableResources res.name
								-. res.amount;
						}
					) produced;
					wS.production <- if wS.working then prod
				...
				| _ -> (
					wS.working <- false;
					{production = List.map (fun r -> {name = r.name; amount = 0.}) prod; produced = []}
				)
			)
			| i when i = Structures.crops -> (
				...
				| Some deposit -> (
					...
					deposit.availableResources <- [{
						name = Resources.food;
						amount = Calculations.get_resource deposit.availableResources Resources.food
			...
			| i when i = Structures.minerCamp -> (
				let wSo = find_worldStructure
					city Structures.seam wS.coords.x [0] in
				let prodSand = [{
						name = Resources.sand;
						amount = Calculations.get_resource prod Resources.sand;
					}] in
				let dtS = (production_time city prodSand city.availableResources time false) in
				...
				let producedSand =
					...
					in
				wS.production <- if dtS = 0. then List.map (fun r -> {name = r.name; amount = 0.}) prodSand else prodSand;
				(match wSo with
					| Some seam -> (
						let prodMetal = [{
						...
						let dtM = min
							(production_time city prodMetal seam.availableResources time true)
							(production_time city prodMetal city.availableResources time false) in
						let producedMetal = List.map
							(fun res -> {name = res.name; amount = res.amount *. dtM})
							prodMetal in
						seam.availableResources <- List.map (
							fun res -> {
								name = res.name;
								amount = Calculations.get_resource seam.availableResources res.name
									-. res.amount;
							}
						) producedMetal;
						wS.working <- wS.working || not (dtM = 0.);
						wS.production <- List.concat [wS.production;
							if not (dtM = 0.) then prodMetal
							else List.map (fun r -> {name = r.name; amount = 0.}) prodMetal];
						{
							production = wS.production;
					...
					| _ -> {
						production = wS.production;
						produced = producedSand;
					}
				)
			)
			| _ -> (
				let dt = production_time
					city prod city.availableResources time false in
				wS.working <- not (dt = 0.);
				let produced =
					if not wS.working then
						[]
						...
						List.map
							...
							prod
					in
				wS.production <- if wS.working then level.production
					else List.map (fun r -> {name = r.name; amount = 0.}) prod;
				{
					production = wS.production;
					produced;
		...
		let can_build city structure x =
			...
			( (* Biome constructible ? *)
				...
				(c.biome = Biomes.ocean && structure = Structures.harbor
			...
			) && (List.for_all (fun a -> a) (* Autres bâtiments qui gênent ? *)
				(List.init structure.width (fun i ->
					(List.for_all (function
						| {coords = {overground}} when overground = o -> false
						| _ -> true) (get city.cells (x+i)).ground
				)))
			) && (List.for_all (function
				| {coords = {overground}; width = 2} when overground = o -> false
				| _ -> true) pc.ground
			)

		let can_buy city (structure, level) =
			let l = structure.levels.(level) in
			List.for_all (fun res -> res.amount >= 0.)
			(Calculations.sum_resources [city.availableResources; l.initialResources])
			&& match structure with
			| i when i = Structures.boat -> Array.exists
				(fun c ->
					List.exists (fun wS -> wS.structure = Structures.harbor)
					c.ground)
				city.cells
			| i when i = Structures.rocketStation || i = Structures.windTurbine -> Array.exists
				(fun c ->
					List.exists (fun wS -> wS.structure = Structures.university && wS.working)
					c.ground)
				city.cells
			| _ -> true
		...
		let update_storage city =
			city.storage <- [];
			Array.iter (fun cell -> List.iter (fun wS ->
				if wS.player's then
					city.storage <- Calculations.sum_resources
						[city.storage; wS.storage];
			) cell.ground) city.cells;
			city.availableResources <- List.map
				(fun r -> {r with amount = min r.amount (Calculations.get_resource city.storage r.name)})
				city.availableResources

		let explore city x =
			let vision, range =
				if (get city.cells x).biome = Biomes.mountain then
					([-2; -1; 1; 2], 2)
				else
					([-4; -3; -2; -1; 1; 2; 3; 4], 4) in
			List.iter
				(fun i -> let cell = get city.cells (x + i) in
					cell.explored <-
					if abs(i) = range then
						max Discovered cell.explored
					else
						if (cell.biome = Biomes.mountain && abs(i) = 1)
							|| cell.biome <> Biomes.mountain then Yes
						else max Discovered cell.explored)
				vision

		let upgrade city wS =
			if wS.level < last wS.structure.levels && can_buy city (wS.structure, wS.level + 1) then (
				let l = wS.structure.levels.(wS.level + 1) in
				city.availableResources <-
					Calculations.sum_resources [city.availableResources; l.initialResources];
				wS.initialResources <- Calculations.sum_resources [wS.initialResources; l.initialResources];
				wS.level <- wS.level + 1;
			...
			if wS.structure = Structures.boat
				&& can_build city Structures.boat (wS.coords.x-1) then (
				...
				wS.coords <- {x = modpos (wS.coords.x-1) city.width;
		...
		let downgrade city wS =
			if wS.level > 0 then (
				let res = List.map (fun r -> {r with amount = -. r.amount})
						(List.filter (fun r -> r.name = Resources.villagers)
						wS.structure.levels.(wS.level).initialResources) in
				city.availableResources <- Calculations.sum_resources [city.availableResources; res];
				wS.initialResources <- Calculations.sum_resources [wS.initialResources; res];
				wS.level <- wS.level -1;
		...
		let build city (structure, level) x =
			if can_buy city (structure, level) && can_build city structure x then (
				...
				cell.ground <- (new_worldStructure (structure, level) x true 0.) :: cell.ground;
				update_storage city;
				explore city x;
			)
		...
		let destroy city wS =
			...
			cell.ground <- List.filter (fun i -> not (i = wS) ||
				(wS.structure = Structures.crops && i.structure = Structures.field)) cell.ground;
			if wS.player's then (
				city.availableResources <- Calculations.sum_resources [city.availableResources;
					List.map (fun r -> {r with amount = -. r.amount})
						(List.filter (fun r -> r.amount > 0. || r.name = Resources.villagers)
						wS.initialResources)];
				update_storage city;
			);
			match city.interactions.toBeBuilt with
			| Some s -> city.interactions.possibleCoords <- Array.mapi
				(fun x cell -> can_build city s x)
				city.cells
			| _ -> ()
		...
		let generate_cells width =
			...
			Array.mapi (fun x biome ->
				let ground = ref [] in
				if biome = Biomes.plain
					&& (x mod 10) >= 5 && (x mod 10) <= 7 then
				...
				if biome = Biomes.desert
					&& (x mod 5) = 0 && (x mod 3) <> 0 then
					ground := (oil x width)::!ground;
				if x = 0 then (
					ground := (new_worldStructure (Structures.cityHall, 0) x true 0.)::!ground;
				);
				if x = 2 then (
				...
				);
				{
					...
					ground = !ground;
					x;
					explored = (if abs(width/2+1-x) >= width/2-3 then Yes
						else if abs(width/2+1-x) >= width/2-4 then Discovered else No);
				}
			) (Array.init width (
				fun x -> match iof (h x) with
					| 0 -> Biomes.ocean;
					| 2 -> Biomes.mountain;
		...
		let new_city () = {
			...
			storage = Calculations.sum_resources [Structures.cityHall.levels.(0).storage; Structures.well.levels.(0).storage];
		...
		let update_city city timeSinceLastUpdate =
			...
			Array.iter (fun cell -> List.iter (fun wS ->
				if wS.player's then (
					let update =
						update_worldStructure city wS timeSinceLastUpdate in
					city.production <- Calculations.sum_resources
						[city.production; update.production];
					city.availableResources <- Calculations.sum_resources
						[city.availableResources; update.produced]
				) else (
					if List.for_all (fun r -> r.amount <= 0.) wS.availableResources then
						destroy city wS
				);
				if Calculations.get_resource city.availableResources Resources.water <= 0.
					|| Calculations.get_resource city.availableResources Resources.food <= 0. then
				game_over city
			) cell.ground) city.cells;
	...
	module Interactions = struct
		...
		let buildingList = [
			...
			Structures.crops;
			Structures.well;
			Structures.minerCamp;
			Structures.lumberCamp;
			Structures.building;
			Structures.harbor;
			Structures.farm;
			Structures.derrick;
			Structures.university;
			Structures.rocketStation;
			Structures.boat;
			Structures.tunnel;
			Structures.coalFiredPlant;
			Structures.windTurbine;
		]

		let want_to_build city s =
			city.interactions.toBeBuilt <-
				if City.can_buy city (s, 0) then Some s else None;
			city.interactions.possibleCoords <- Array.mapi
				(fun x cell -> City.can_build city s x)
				city.cells

		let can_buy city structure =
			City.can_buy city (structure, 0)
		...
		let can_delete city =
			List.exists (fun wS -> wS.editable &&
				List.for_all (fun res -> res.amount >= 0.)
				(Calculations.sum_resources [city.availableResources;
					List.map (fun r -> {r with amount = -. r.amount})
		...
		let show_up_button city =
			List.exists (fun wS -> wS.editable
				...
				|| wS.structure = Structures.boat) city.interactions.selection

		let can_up city =
			List.exists (fun wS -> wS.editable && wS.working
				&& wS.level < last wS.structure.levels
		...
		let show_move_button city =
			List.exists (fun wS -> wS.structure = Structures.boat)
				city.interactions.selection

		let can_down city =
			show_down_button city
		...
		let up city =
			if can_up city then
				City.upgrade city (List.hd city.interactions.selection)
	...
	let update game timeSinceLastUpdate =
		for i = 0 to last game.cities do
			...
			if city.state = Playing then
				City.update_city city timeSinceLastUpdate
		done
