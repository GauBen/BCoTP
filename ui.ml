module UserInterface = struct
	...
	and ui = {
		...
		scene: scene;
		hud: hud;
		mutable tick: int; (* Pour les animations *)
	...
	and screen = {element: uiElement}

	(* Scène / Plateau de jeu *)
	and scene = {
		mutable cameraX: float; (* Position horizontale de la caméra *)
		mutable tremor: pixels; (* Pour un effet de secousse *)
		mutable scale: int; (* Taille (en pixels) d'un carré du jeu *)
	...
	module Shaders = struct
		...
		let dark i j c = if c = 0 then 0 else 1
		let disabled i j c =
	...
	module Pixels = struct
		let scene = 6
		let per_square ui = ui.scene.scale
		let per_building ui = ui.scene.scale * Resources.Widths.building
		let per_block ui = ui.scene.scale * Resources.Widths.block
		let surface ui = ui.scene.element.sizes.y / 3
		let curvature scene' ui = scene'.sizes.y / 10
		let hud_scale screen = 2 + screen.element.sizes.y / 400
		let hud_height scale = (2+(Resources.Font.y+2)*Game.Resources.length)*scale
		let resource_panel_width scale =
			(2*2+Resources.Font.y+9*Resources.Font.x+9+3)*scale
		let building_panel_width scale =
			(3*2+(Resources.Widths.button+2)*Layout.buildingPanelCols-2)*scale
		...
		let hud_margin width scale =
			width / 2 - (resource_panel_width scale+info_panel_width scale
				+building_panel_width scale+2)/2
	end (* / Pixels *)
	...
	module Calculations = struct
		...
		let most_distant_blocks scene' ui =
			...
			let delta = foi scene'.sizes.x /.
				(2. *.foi (Pixels.per_block ui)) in
			iof (floor (cx -. delta)) - 1, iof (floor (cx +. delta))

		let mouse_game_coords scene' ui : Game.coords =
			let mx = ui.mouse.elementPosition.x in
			let x = iof (floor (ui.scene.cameraX +. foi (2*mx
				-  scene'.sizes.x) /. (2. *. foi (Pixels.per_block ui)))) in
			let curvation = curve scene' ui mx in
			{x; overground = ui.mouse.elementPosition.y
				>= Pixels.surface ui + curvation}

	end (* / Calculations *)

	module Game2Resources = struct

		let structure (s: Game.structure) = match s with
			...
			| i when i = Game.Structures.rocketStation -> Resources.Structures.rocketStation
			| i when i = Game.Structures.coalFiredPlant -> Resources.Structures.coalFiredPlant
			| i when i = Game.Structures.windTurbine -> Resources.Structures.windTurbine
			| _ -> failwith "Game2Resources / Structure inconnue"

		let icon name = match name with
		...
		let biome b1 b2 = match b1, b2 with
			| i, j when i == Game.Biomes.plain && j == Game.Biomes.plain -> Resources.Biomes.plainPlain
			| i, j when i == Game.Biomes.plain && j == Game.Biomes.desert -> Resources.Biomes.plainDesert
			| i, j when i == Game.Biomes.plain && j == Game.Biomes.ocean -> Resources.Biomes.plainOcean
			| i, j when i == Game.Biomes.plain && j == Game.Biomes.mountain -> Resources.Biomes.plainMountain
			| i, j when i == Game.Biomes.desert && j == Game.Biomes.plain -> Resources.Biomes.desertPlain
			| i, j when i == Game.Biomes.desert && j == Game.Biomes.desert -> Resources.Biomes.desertDesert
			| i, j when i == Game.Biomes.desert && j == Game.Biomes.ocean -> Resources.Biomes.desertOcean
			| i, j when i == Game.Biomes.desert && j == Game.Biomes.mountain -> Resources.Biomes.desertMountain
			| i, j when i == Game.Biomes.ocean && j == Game.Biomes.plain -> Resources.Biomes.oceanPlain
			| i, j when i == Game.Biomes.ocean && j == Game.Biomes.desert -> Resources.Biomes.oceanDesert
			| i, j when i == Game.Biomes.ocean && j == Game.Biomes.ocean -> Resources.Biomes.oceanOcean
			| i, j when i == Game.Biomes.ocean && j == Game.Biomes.mountain -> Resources.Biomes.oceanMountain
			| i, j when i == Game.Biomes.mountain && j == Game.Biomes.plain -> Resources.Biomes.mountainPlain
			| i, j when i == Game.Biomes.mountain && j == Game.Biomes.desert -> Resources.Biomes.mountainDesert
			| i, j when i == Game.Biomes.mountain && j == Game.Biomes.ocean -> Resources.Biomes.mountainOcean
			| i, j when i == Game.Biomes.mountain && j == Game.Biomes.mountain -> Resources.Biomes.mountainMountain
			| _ -> failwith "Game2Resources / Biome inconnu"

		let button (s: Game.structure) = match s with
			...
			| i when i = Game.Structures.rocketStation -> Resources.Buttons.rocketStation
			| i when i = Game.Structures.coalFiredPlant -> Resources.Buttons.coalFiredPlant
	...
	module Draw = struct
		...
		let _fill_rect_game_fixed scene' ui {x; y} =
			_fill_rect
				{x = x + scene'.position.x; y = y + scene'.position.y}
		...
		let draw_bitmap bitmap {x; y} scale =
			_draw_bitmap
				(_fill_rect, (fun x -> 0), Shaders.no)
				bitmap {x; y} scale

		let _write_digit ui n {x; y} =
			draw_bitmap Resources.Font.numbers.(n) {x; y} ui.hud.scale

		let _draw_character ui c {x; y} =
			draw_bitmap c {x; y} ui.hud.scale
		...
		let draw_number ui n {x; y} (signed, rightAligned, pad) =
			...
			let offset =
				(if rightAligned > 0 && pad == None then rightAligned - l else 0) in
			if sign = 1 then
				_draw_character ui
		...
		let _draw_on_scene scene' ui (shade) bitmap {x; y} =
			_draw_bitmap
				...
				bitmap {x; y} ui.scene.scale

		let _draw_gif scene' ui (shade)
				(gif : Resources.gif) ({x; overground} : Game.coords) =
			let bitmap = gif.frames.(ui.tick mod gif.length) in
			let gc = Pixels.per_block ui in
			_draw_on_scene scene' ui (shade) bitmap
				{
					x = iof (foi scene'.sizes.x /. 2. -. ui.scene.cameraX *. foi gc) + x * gc;
					y = Pixels.surface ui + ui.scene.scale * if overground then 0 else -len bitmap;
				}
		...
		let _draw_shaded_block scene' ui (top, bottom) (block : Resources.block) x =
			(match block with
			| {overground = Some gif; _} ->
				_draw_overground scene' ui (top) gif x
			| _ -> ());
			(match block with
			| {underground = Some gif; _} ->
				_draw_underground scene' ui (bottom) gif x
			| _ -> ())

		let _draw_block scene' ui =
			_draw_shaded_block scene' ui (Shaders.no, Shaders.no)

		let _draw_shaded_structure
				scene' ui (top, bottom) ((s: Game.structure), level) x =
		...
		let draw_sky scene' ui =
			...
			for x = 0 to (rightMost - leftMost + 1) * Resources.Widths.block do
				let posx = iof (foi scene'.sizes.x /. 2. -. cx *. foi gc)
					+ leftMost * gc + x * scale in
				if -scale <= posx && posx <= scene'.sizes.x then
					_fill_rect_game scene' ui {x = posx; y = -scene'.position.y-1}
		...
		let draw_ground scene' ui =
			...
			for x = leftMost to rightMost do
				...
				let shader = match cell.explored with
					| Yes -> Shaders.no, Shaders.no
					| Discovered -> Shaders.dimmed, Shaders.dimmed
					| No -> Shaders.dark, Shaders.dark in
			...
			for x = leftMost to rightMost do
				...
				let shader = match cell.explored with
					...
					| Discovered -> Shaders.dimmed, Shaders.dimmed
					| No -> Shaders.dark, Shaders.dark in
				List.iter (fun (s: Game.worldStructure) ->
					_draw_shaded_structure
						scene' ui
		...
		let draw_hints scene' ui =
			match scene' == ui.hoveredElement,
				ui.city.interactions.toBeBuilt with
			| true, Some s ->
				let {x; overground} : Game.coords
					= Calculations.mouse_game_coords scene' ui in
				if overground = s.overground then
					let shaders = if get ui.city.interactions.possibleCoords x
						then (Shaders.checkerBoardOk, Shaders.checkerBoardOk)
						else (Shaders.checkerBoardNope, Shaders.checkerBoardNope) in
		...
		let _draw_hud_availableRes element ui =
			...
			let draw thing (x2, y2) = draw_bitmap thing {x=x+x2; y=y+y2} ui.hud.scale in
			List.iter (fun (aR: Game.resource) ->
				let y2 = (2 + (Game.Resources.length-aR.name)*(Resources.Font.y+2))*ui.hud.scale in
				draw (Game2Resources.icon aR.name)
					(2*ui.hud.scale, y+y2);
				draw_padded_number ui (iof aR.amount) 4 {x=x+(4+Resources.Font.y)*ui.hud.scale; y=y+y2};
				draw Resources.Font.slash
					((4+Resources.Font.x*4+3+Resources.Font.y+2)*ui.hud.scale, y+y2);
			) ui.city.availableResources;
			List.iter (fun (s: Game.resource) ->
				let y2 = (2 + (Game.Resources.length-s.name)*(Resources.Font.y+2))*ui.hud.scale in
				draw_padded_number ui (iof s.amount) 4
					{x=x+(4+Resources.Font.x*5+3+2+Resources.Font.y+2)*ui.hud.scale;y= y+y2};
			) ui.city.storage
		...
		let _draw_hud_production element ui =
			...
			_fill_rect {x; y} element.sizes;
			let draw thing (x2, y2) = draw_bitmap thing {x=x+x2; y=y+y2} ui.hud.scale in
			List.iter (fun (p: Game.resource) ->
				let y2 = (2 + (Game.Resources.length-p.name)*(Resources.Font.y+2))*ui.hud.scale in
				if p.name = Game.Resources.villagers then (
					draw
						Resources.Icons.clock
						(2*ui.hud.scale, y+y2);
					let nb n x2 = draw_padded_number_zeroes
						ui n 2
						{x=x+(4+Resources.Font.y+3+x2*(Resources.Font.x+1))*ui.hud.scale;y= y+y2} in
					nb (iof ui.city.time / 3600 mod 100) 0;
					nb (iof ui.city.time / 60 mod 60) 3;
					nb (iof ui.city.time mod 60) 6;
					let colon x2 = draw
						Resources.Font.colon
						((4+Resources.Font.y+3+x2*(Resources.Font.x+1))*ui.hud.scale, y+y2) in
					colon 2; colon 5
				) else (
					draw
						(Game2Resources.icon p.name)
						(2*ui.hud.scale, y+y2);
					draw_padded_signed_number
						ui (iof (p.amount *. 60.)) 4 {x=x+(4+Resources.Font.y)*ui.hud.scale; y=y+y2};
					draw
						Resources.Font.slash
						((4+Resources.Font.x*4+3+Resources.Font.y+2)*ui.hud.scale, y+y2);
					draw
						Resources.Font.min
						((4+Resources.Font.x*5+3+2+Resources.Font.y+2)*ui.hud.scale, y+y2);
				)
			) ui.city.production

		let draw_hud_resources element ui =
			(if ui.hud.showProduction
				then _draw_hud_production
				else _draw_hud_availableRes) element ui

		let draw_hud_buildings element ui =
			Graphics.set_color Resources.palette.(22);
		...
		let draw_hud_info box' ui =
			...
			if List.length ui.city.interactions.selection >= 1 then (
				let y2 i = box'.sizes.y + box'.position.y
					- ui.hud.scale * (i+1) * (Resources.Font.y+2) in
				...
				draw_padded_number ui lvl 1
					{x = x + 28*ui.hud.scale; y = y2 0};
				let i = ref 1 in
				List.iter
						...
						if wS.player's then (
							List.iter
								(fun (initialResource: Game.resource) ->
									if initialResource.amount > 0.
										|| initialResource.name = Game.Resources.villagers then (
									draw_bitmap
										(Game2Resources.icon initialResource.name)
										{x=x+2*ui.hud.scale; y=y+(y2 !i)}
										ui.hud.scale;
									draw_padded_signed_number
										ui (iof initialResource.amount) 4
										{x=x+(4+Resources.Font.y)*ui.hud.scale; y=y+y2 !i};
									incr i;
								))
								wS.initialResources;
							List.iter
								(fun (p: Game.resource) ->
									draw_bitmap
										...
										ui.hud.scale;
									draw_padded_signed_number
										ui (iof (p.amount *. 60.)) 4 {x=x+(4+Resources.Font.y)*ui.hud.scale; y=y+y2 !i};
									_draw_character
										ui Resources.Font.slash
										{x=x+(4+Resources.Font.x*4+3+Resources.Font.y+2)*ui.hud.scale; y=y+y2 !i};
									_draw_character
										ui Resources.Font.min
										{x=x+(4+Resources.Font.x*5+3+2+Resources.Font.y+2)*ui.hud.scale; y=y+y2 !i};
									incr i;
								)
								wS.production
						) else List.iter
							(fun (initialResource: Game.resource) ->
								...
								let storage = Game.Calculations.get_resource
									wS.initialResources initialResource.name in
								draw_bitmap
									(Game2Resources.icon initialResource.name)
									{x=x+2*ui.hud.scale; y=y+(y2 !i)}
									ui.hud.scale;
								...
								draw_padded_number
									ui (iof storage) 4
									{x=x+(4+Resources.Font.x*5+3+Resources.Font.y+4)*ui.hud.scale; y=y+y2 !i};
								incr i;
		...
		let draw_delete_button button' ui =
			if Game.Interactions.show_delete_button ui.city then (
				draw_bitmap (match button' == ui.hoveredElement, Game.Interactions.can_delete ui.city with
		...
		let draw_up_button button' ui =
			if Game.Interactions.show_up_button ui.city then (
				draw_bitmap (match button' == ui.hoveredElement, Game.Interactions.can_up ui.city with
					| _, false -> Resources.SmallButtons.backgroundDisabled
					| true, _ -> Resources.SmallButtons.backgroundActive
					| _ -> Resources.SmallButtons.background)
					button'.position ui.hud.scale;
				draw_bitmap (if Game.Interactions.show_move_button ui.city then
					Resources.SmallButtons.left else Resources.SmallButtons.up) button'.position ui.hud.scale
			)

		let draw_down_button button' ui =
			if Game.Interactions.show_down_button ui.city then (
				draw_bitmap (match button' == ui.hoveredElement, Game.Interactions.can_down ui.city with
					| _, false -> Resources.SmallButtons.backgroundDisabled
					| true, _ -> Resources.SmallButtons.backgroundActive
					| _ -> Resources.SmallButtons.background)
					button'.position ui.hud.scale;
				draw_bitmap (if Game.Interactions.show_move_button ui.city then
					Resources.SmallButtons.right else Resources.SmallButtons.down) button'.position ui.hud.scale
			)
		...
		let draw_building_button s button' ui =
			draw_bitmap (
				match
					...
					Game.Interactions.can_buy ui.city s with
		...
		let draw_infoTip (p: Game.resources) tip' ui =
			...
			List.iteri (fun i (p: Game.resource) ->
				let y2 = y + (2+(Resources.Font.y+2)*(l-i))*ui.hud.scale in
				draw_bitmap (Game2Resources.icon p.name)
					{x = x + 2*ui.hud.scale; y = y2} ui.hud.scale;
		...
		let draw_element ui element =
			...
			and explore_children children =
				match children with
					| [] -> []
					| child::q -> explore_element child @ explore_children q in
	...
	module EventHandlers = struct
		...
		let play e ui =
			ui.screen.element.children <- [ui.hud.element; ui.scene.element];
			ui.scene.cameraX <- 1.5;
			ui.city.state <- Playing

		let scene_click element ui =
			let coords =
				Calculations.mouse_game_coords element ui in
			Game.Interactions.click ui.city coords;
			()

		let hud_resources_click element ui =
			ui.hud.showProduction <- not ui.hud.showProduction

		let building_button_click s element ui =
			Game.Interactions.want_to_build ui.city s

		let building_button_over s infoTip' button' ui =
			ui.hud.infoTip <- Some infoTip';
			ui.hud.element.children <- infoTip'::ui.hud.element.children

		let building_button_move s button' ui =
			match ui.hud.infoTip with
				| Some e -> e.position <- {x=ui.mouse.position.x+10;y=ui.mouse.position.y+10};
				| _ -> ()

		let building_button_out s button' ui =
			match ui.hud.infoTip with
				| Some e -> ui.hud.element.children <-
					List.filter (fun a -> not (a == e)) ui.hud.element.children;
					ui.hud.infoTip <- None
				| _ -> ()

		let delete_button_over button' ui =
			if Game.Interactions.show_delete_button ui.city then (
				let p: Game.resources = Game.Interactions.earned_on_destroyed ui.city in
				if List.length p >= 1 then (
					let infoTip' = {
						position = {x=0;y=0};
						sizes = {x=Pixels.infoTip_width ui.hud.scale;
							y=(2+(Resources.Font.y+2)*(List.length p))*ui.hud.scale};
		...
		let delete_button_move button' ui =
			match ui.hud.infoTip with
				| Some e -> e.position <- {x=ui.mouse.position.x+10;y=ui.mouse.position.y+10};
				| _ -> ()

		let delete_button_out button' ui =
			match ui.hud.infoTip with
				| Some e -> ui.hud.element.children <-
					List.filter (fun a -> not (a == e)) ui.hud.element.children;
					ui.hud.infoTip <- None
		...
		let delete_button_click button' ui =
			Game.Interactions.destroy ui.city

		let up_button_over button' ui =
			if Game.Interactions.show_up_button ui.city
				&& not (Game.Interactions.show_move_button ui.city) then (
		...
		let down_button_over button' ui =
			if Game.Interactions.show_down_button ui.city
				&& not (Game.Interactions.show_move_button ui.city) then (
		...
		let up_button_move button' ui =
			delete_button_move button' ui

		let up_button_out button' ui =
			delete_button_out button' ui

		let up_button_click button' ui =
			Game.Interactions.up ui.city

		let down_button_move button' ui =
			delete_button_move button' ui

		let down_button_out button' ui =
			delete_button_out button' ui

		let down_button_click button' ui =
			Game.Interactions.down ui.city
	...
	module Elements = struct
		...
		let new_scene screen (hud: hud) = {
			...
			sizes = {
				x = screen.element.sizes.x;
				y = screen.element.sizes.y - hud.element.sizes.y;
			};
		...
		let new_hud_resources (hud: hud) = {
			...
			sizes = {
				x = Pixels.resource_panel_width hud.scale;
				y = hud.element.sizes.y;
			};
		...
		let new_infoTip (p : Game.resources) (hud : hud) = {
			position = {x=0;y=0};
			sizes = {x=Pixels.infoTip_width hud.scale;
		...
		{
			position = {
				x = hudBuildings.position.x +
					(3 + (Resources.Widths.button+2)*x) * hud.scale;
				y = hudBuildings.position.y +
					(3 + (Resources.Widths.button+2)*(Layout.buildingPanelRows - y - 1)) * hud.scale;
			};
			sizes = {
			...
			onMouseOver = Some (EventHandlers.building_button_over s (new_infoTip (s.levels.(0).initialResources) hud));
		...
		let new_hud_buildings (hud: hud) =
			let hudBuildings = {
				position = {x = hud.element.position.x + hud.scale + Pixels.resource_panel_width hud.scale; y = 0};
				sizes = {
					x = Pixels.building_panel_width hud.scale;
					y = hud.element.sizes.y;
				};
		...
		let new_delete_button (hud: hud) info = {
			position = {x = info.position.x + 4*hud.scale; y = info.position.y + 2*hud.scale};
			sizes = {
		...
		let new_up_button (hud: hud) info = {
			position = {
				x = info.position.x+Resources.Widths.smallButton*hud.scale+6*hud.scale;
		...
		let new_down_button (hud: hud) info = {
			position = {
				x = info.position.x+2*Resources.Widths.smallButton*hud.scale+8*hud.scale;
				y = info.position.y + 2*hud.scale;
			};
			sizes = {
				x = Resources.Widths.smallButton*hud.scale;
		...
		let new_hud_info (hud: hud) = let info = {
				position = {x = hud.element.position.x + 2*hud.scale + Pixels.resource_panel_width hud.scale
					+ Pixels.building_panel_width hud.scale; y = 0};
				sizes = {
					x = Pixels.info_panel_width hud.scale;
					y = hud.element.sizes.y;
				};
			...
			info.children <- [new_delete_button hud info;
				new_up_button hud info; new_down_button hud info];
			info
		...
		let new_hud screen =
			...
			let hud = {
				...
				element = {
					position = {x = Pixels.hud_margin screen.element.sizes.x scale; y = 0};
			...
			hud.element.children <- [new_hud_resources hud;
				new_hud_info hud;new_hud_buildings hud];
			hud
		...
		let new_splash screen = {
			position = screen.element.position;
			sizes = screen.element.sizes;
	...
	module Events = struct
		...
		let get_hovered_element ui =
			...
			let is_focused el = match el with
				| {
					position = {x = x; y = y};
					sizes = {x = width; y = height};
				} -> x <= mx && mx < x + width && y <= my && my < y + height in
			let rec explore_element element =
				explore_children element.children @ [element]
			and explore_children children =
				match children with
				| [] -> []
				| child::q -> explore_element child @ explore_children q in
			List.find is_focused (explore_element ui.screen.element)

		let trigger_mouseMove element ui =
			match element with
			| {onMouseMove = Some handler} -> handler element ui
			| _ -> ()
		let trigger_mouseOver element ui =
			match element with
			| {onMouseOver = Some handler} -> handler element ui
			| _ -> ()
		let trigger_mouseOut element ui =
			match element with
			| {onMouseOut = Some handler} -> handler element ui
			| _ -> ()
		let trigger_mouseDown element ui =
			match element with
			| {onMouseDown = Some handler} -> handler element ui
			| _ -> ()
		let trigger_mouseUp element ui =
			match element with
			| {onMouseUp = Some handler} -> handler element ui
			| _ -> ()
		let trigger_mouseClick element ui =
			match element with
			| {onMouseClick = Some handler} -> handler element ui
			| _ -> ()
		...
		let mouse ui =
			...
			let element = try get_hovered_element ui with Not_found -> ui.screen.element in
			ui.mouse.elementPosition <- {
				x = ui.mouse.position.x - element.position.x;
				y = ui.mouse.position.y - element.position.y;
			};
			match ui.hoveredElement == element,
				ui.clickedElement == element,
				ui.mouse.down,
				Graphics.button_down () with
			| false, _, false, false
	...
	let new_ui city =
		...
		let scene = {
			...
			tremor = {x = 0; y = 0};
			selection = None;
			element = Elements.new_scene screen hud;
		} in
		let ui = {
			city;
			scene;
			...
			tick = 0;
			screen;
			clickedElement = screen.element;
			hoveredElement = screen.element;
		} in
		ui.screen.element.children <- [ui.hud.element; ui.scene.element];
		if city.state = Paused then
			ui.screen.element.children <- [Elements.new_splash screen];
		ref ui

	let update refUi =
		let ui = !refUi in
		if ui.screen.element.sizes.x <> sx ()
			|| ui.screen.element.sizes.y <> sy () then (
			let ui = !(new_ui ui.city) in
			refUi := ui;
		);
		Events.mouse ui;
		if ui.mouse.position.x = 0
			then ui.scene.cameraX <- ui.scene.cameraX -. 0.2;
		if ui.mouse.position.x = ui.screen.element.sizes.x - 1
			then ui.scene.cameraX <- ui.scene.cameraX +. 0.2;
		while Graphics.key_pressed () do
			let k = ord (Graphics.read_key ()) in
			if KeyboardControls.is_left k
				then ui.scene.cameraX <- ui.scene.cameraX -. 0.1;
			if KeyboardControls.is_right k
				then ui.scene.cameraX <- ui.scene.cameraX +. 0.1;
			if KeyboardControls.is_zoom_in k
				then ui.scene.scale <- ui.scene.scale + 1;
			if KeyboardControls.is_zoom_out k
				then ui.scene.scale <- ui.scene.scale - 1;
			if KeyboardControls.is_prod k
				then ui.hud.showProduction <- not ui.hud.showProduction;
			if KeyboardControls.is_esc k
				then Game.Interactions.nothing ui.city;
		done;
		if ui.city.state = Won || ui.city.state = GameOver then (
			ui.hud.element.children <- [];
		);
		ui.scene.scale <- max 1 (min ui.scene.scale 12);
	...
	let draw refUi =
		let ui = !refUi in
		if ui.screen.element.sizes.x <= 790 || ui.screen.element.sizes.y <= 550 then (
			Graphics.moveto 10 10;
			Graphics.set_color Resources.palette.(1);
			Graphics.draw_string ("Fenetre trop petite / Ouvrir en 800x600");
		) else (
			Draw.draw_element ui ui.screen.element
		)
